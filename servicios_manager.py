...
class ServiciosManager:
    ...
    def crear_servicio(self, datos_servicio, files=None):
        ...
        data, error = self.client.post(Endpoints.SERVICIOS, datos_servicio, files=files)
        if error:
    ...
    def actualizar_servicio(self, servicio_id, datos_servicio, files=None):
        """Actualizar un servicio existente"""
        # Convertir impuestos a formato JSON si es necesario
        if 'servicioimpuesto_set' in datos_servicio and isinstance(datos_servicio['servicioimpuesto_set'], list):
            datos_servicio['servicioimpuesto_set'] = json.dumps(datos_servicio['servicioimpuesto_set'])
           
        data, error = self.client.patch(f"{Endpoints.SERVICIOS}{servicio_id}/", datos_servicio, files=files)
        if error:
